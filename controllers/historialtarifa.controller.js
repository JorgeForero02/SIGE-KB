...
const HistorialTarifa = db.HistorialTarifa;

const HistorialTarifaController = {
  // Obtener todos
  getAll: async (req, res, next) => {
    try {
      const items = await HistorialTarifa.findAll();
  ...
  getById: async (req, res, next) => {
    try {
      const { id } = req.params;
      const item = await HistorialTarifa.findByPk(id);
  ...
  create: async (req, res, next) => {
    try {
      const newItem = await HistorialTarifa.create(req.body);
...
module.exports = HistorialTarifaController;
