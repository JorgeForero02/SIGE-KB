...
const CategoriaEgreso = db.CategoriaEgreso;

const CategoriaEgresoController = {
  // Obtener todos
  getAll: async (req, res, next) => {
    try {
      const items = await CategoriaEgreso.findAll();
  ...
  getById: async (req, res, next) => {
    try {
      const { id } = req.params;
      const item = await CategoriaEgreso.findByPk(id);
  ...
  create: async (req, res, next) => {
    try {
      const newItem = await CategoriaEgreso.create(req.body);
